Character = Class.extend({
    ...
    aStarPathFinder: function() {
        ...
        while (openList.length > 0) {
            ...
            var bestNode = this.getNeighbor(currentNode);
            var currentNode = bestNode;

            //check to see if we reached destination
            if (currentNode === this.destination) {
                var path = [this.destination]; //Initialize path with destination node
    ...
    getNeighbor: function(currentNode) {
        ...
        for (var i = 0; i < possibleNeighbors.length; i++) {
            ...
            if (Fscore < bestScore) {
                var neighbor = {};
                neighbor.characterCoords = {'top': possibleNeighbors[i][1], 'bottom': possibleNeighbors[i][1] + gGameEngine.tileSize, 'right': possibleNeighbors[i][0] + gGameEngine.tileSize, 'left': possibleNeighbors[i][1]};
        ...
        for (var j = 0; j < possibleNeighbors.length; j++) {
            if (possibleNeighbors[j] != bestNeighbor) {
                this.openList.push(possibleNeighbors[j]);
            }
        }
        this.closedList.push(bestNeighbor);
        return bestNeighbor;
    }
